...
- миграции содержат `001`–`016`, `019`–`029`;
...
## Smoke-тест баг-репортов

- [ ] В welcome-сообщении бота есть кнопка «Сообщить об ошибке».
- [ ] Тестировщик может отправить описание, скриншот или видео.
- [ ] Владелец получает уведомление с номером ошибки.
- [ ] В Mini App → «Ещё → Сообщить об ошибке» создаётся репорт.
- [ ] Репорту можно назначить статусы `new`, `in_progress`, `fixed`, `closed`.
