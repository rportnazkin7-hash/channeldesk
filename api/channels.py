...
def pending_connections(user:dict=Depends(current_user)):
    with connect() as conn,conn.cursor() as cur:
        ...
        FROM cd_channel_connections WHERE actor_telegram_id=%s AND status='pending' ORDER BY updated_at DESC""",(user['telegram_id'],))
        return cur.fetchall()
...
def connect_channel(workspace_id:int,payload:ConnectChannel,user:dict=Depends(current_user)):
    ...
    with connect() as conn,conn.cursor() as cur:
        cur.execute("""SELECT * FROM cd_channel_connections WHERE id=%s AND actor_telegram_id=%s AND status='pending' FOR UPDATE""",(payload.connection_id,user['telegram_id']))
        pending=cur.fetchone()
        if not pending: raise HTTPException(404,'Канал не найден. Переустановите права бота в канале')
        permissions=pending.get('bot_permissions') or {}
        if not permissions.get('can_post_messages'):
            raise HTTPException(422,'Боту не выдано право публикации сообщений')
        cur.execute('SELECT id,workspace_id FROM cd_channels WHERE telegram_chat_id=%s',(pending['telegram_chat_id'],)); existing=cur.fetchone()
        if existing and existing['workspace_id']!=workspace_id: raise HTTPException(409,'Канал уже подключён к другому рабочему пространству')
        if existing:
            ...
            (pending['title'],pending['username'],__import__('json').dumps(permissions),user['id'],existing['id']))
        else:
            ...
            (workspace_id,pending['telegram_chat_id'],pending['title'],pending['username'],__import__('json').dumps(permissions),user['id']))
        channel=cur.fetchone()
        cur.execute("UPDATE cd_channel_connections SET status='connected',connected_channel_id=%s,updated_at=now() WHERE id=%s",(channel['id'],pending['id']))
        cur.execute("""INSERT INTO cd_audit_log(workspace_id,user_id,action,entity_type,entity_id,details)
        VALUES(%s,%s,'channel.connected','channel',%s,%s::jsonb)""",(workspace_id,user['id'],channel['id'],'{}'))
        return channel
